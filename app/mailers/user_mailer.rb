class UserMailer < ActionMailer::Base
  ...
  #
  def apply_claim(account_id, claim_id, employee_id)
  ...
  def claim_approval(account_id, claim_id, employee_id)
    @account = Account.find(account_id)
    @claim = Claim.find(claim_id)
    @employee = Employee.find(employee_id)
  ...
  def apply_leave(account_id, leave_id, employee_id)
    @account = Account.find(account_id)
  ...
  def leave_approval(account_id, leave_id, employee_id)
    @account = Account.find(account_id)
    @leave = Leave.find(leave_id)
    @employee = Employee.find(employee_id)
