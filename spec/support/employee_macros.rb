module EmployeeMacros
  ...
  def create_employee_cancan(can_approve_leave, can_approve_claim)
    ...
    position = Position.new(name: "Clerk", max_claims_cents: 240000, can_approve_leave: can_approve_leave, can_approve_claim: can_approve_claim)
    position.save
  
    employee = Employee.new(first_name: "Test", last_name: "lee", available_claims_cents: 150000, can_self_approve: true)
    employee.account_id = account.id
    employee.organization_id = organization.id
    employee.position_id = position.id
    employee.save
